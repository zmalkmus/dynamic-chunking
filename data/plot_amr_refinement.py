...
for r_dir in r_dirs:
    usage_path = os.path.join(base_dir, r_dir, 'amr', 'usage.txt')

    if not os.path.exists(usage_path):
        print(f"Skipping {r_dir} (no amr/usage.txt found)")
        continue
...
plt.xlabel('Timestep')
plt.ylabel('Usage (MB)')
plt.title('Disk Usage per Timestep (AMR)')
plt.xlim(left=0)
plt.ylim(bottom=0)
plt.grid(True)
plt.legend(title="Refinement Level", fontsize=12)
plt.tight_layout()

# Save the single combined plot
plt.savefig('all_refinement_amr_usage.png')
plt.show()
