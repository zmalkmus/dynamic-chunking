...
def combine_timestep_files(input_dir, output_dir):
    """
    combines miniAMR dump files (dump_ts*_pe*.dat) into single checkpoint files per timestep.

    Args:
        input_dir (str): Directory containing the dump_*.dat files.
        output_dir (str): Directory where combined checkpoint_*.dat files will be saved.
    """
    ...
    # Assumes format dump_tsXXXX_peYYYYYY.dat
    file_pattern = re.compile(r"dump_ts(\d+)_pe(\d+)\.dat$")

    # Group files by timestep
    timestep_files = defaultdict(list)
    search_path = os.path.join(input_dir, "dump_ts*_pe*.dat")
    all_dump_files = glob.glob(search_path)

    if not all_dump_files:
        print(f"Warning: No 'dump_ts*_pe*.dat' files found in '{input_dir}'.")
        return
    ...
    for timestep, files in sorted(timestep_files.items()):
        ...
        output_filename = f"checkpoint_ts{timestep:04d}.dat"
        output_filepath = os.path.join(output_dir, output_filename)

        print(f"Comdating {len(files)} files for timestep {timestep} into {output_filepath}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_dir", help="Directory containing the dump_ts*_pe*.dat files.")
    parser.add_argument("output_dir", help="Directory to save the combined checkpoint_ts*.dat files.")
