...
def plot(df, full_avg):
    ...
    # Only include the 2048 row.
    chunk_sizes = [2048]
    dedup_cases = ["0_100_0", "25_25_50", "80_10_10", "100_0_0"]
    
    # NEW: Removed "Tree_Ifstream" and "Tree_Liburing" from these orders
    method_order = [
        "basic",
        "list",
        "Tree_Baseline",
    ...
    # Create an outer GridSpec with 1 row x 4 columns.
    fig = plt.figure(figsize=(15, 8))
    outer = gridspec.GridSpec(1, 4, wspace=0.2, hspace=0.1)
    
    # Iterate over dedup_cases.
    for row_idx, cs in enumerate(chunk_sizes):
        ...
        for col_idx, d_case in enumerate(dedup_cases):
            ...
            if not df_sub.empty:
                # Group data by method and compute mean and std.
                stats = df_sub.groupby("method")["runtime"].agg(["mean", "std"])
                # Reindex using the fixed order and drop missing methods.
            ...
            if full_avg < y_break:
                ...
                # Covers the edge case if full_avg == y_break
                ax_lower.axhline(full_avg, color='black', linestyle='-', linewidth=1)
                ax_upper.axhline(full_avg, color='black', linestyle='-', linewidth=1)
            ...
            # Optionally remove y-ticks from the upper panel if no data above the break.
            if max_val <= y_break:
                ax_upper.set_yticks([])
    ...
    # Also remove "Tree_Ifstream" and "Tree_Liburing" from the legend
    legend_order = [
        "basic",
        "list",
        "Tree_Baseline",
    ...
    rename_map = {"list": "List", "basic": "Basic", "Tree_Baseline": "Tree_Naive"}
    legend_handles = [
        mpatches.Patch(
    ...
    fig.text(0.08, 0.5, "Average Runtime Over 10 Iterations (s)", va='center', rotation='vertical', fontsize=14)
    fig.suptitle("Restoring Latest Checkpoint (1GB × 16 Files, 2048 B Chunks)", fontsize=20)
    
    # Save the plot to the specified directory before showing.
    plt.savefig("/home/zmalk/Research/ANL/checkpoint_restore/checkpoint_restore_analysis/graphs/graph2.png", bbox_inches="tight")
    plt.show()
...
def main():
    ...
    df = pd.read_csv(
        ...
        header=0,
        names=[
            "seed", "method", "num_files", "data_len", "chunk_size_input",
            "num_chunks", "dedup_case", "i", "chunk_size", "runtime"
        ]
    )
    plot(df, full_avg)
